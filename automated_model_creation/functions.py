...
def custom_cv_eval(df, m, labels, odds, min_ev=0, verbose=False, get_total=True):
    ...
    for train_index, test_index in kf.split(X):
        ...
        # print(X_test[:, -2])
        X_odds = list(
            zip(odds_test[:, -2], odds_test[:, -1], probs[:, 0], probs[:, 1], y_test)
        )
        ev_prepped_df = pd.DataFrame(
            X_odds, columns=["t1_odds", "t2_odds", "t1_prob", "t2_prob", "winner"]
        )
        # display(ev_prepped_df)
        # display(temp_df)
        # print(f"{count}: {get_ev_from_df(ev_prepped_df, print_stats = False)}")
        count = count + 1
        running_total = running_total + get_ev_from_df(
            ev_prepped_df, print_stats=verbose, min_ev=min_ev, get_total=get_total
        )
        # display(ev_prepped_df)
...
def get_ev_for_optimize_mov(
    df_odds, probs, labels, print_stats=False, min_ev=0, get_total=True
):
...
def custom_cv_eval_mov(df, m, labels, odds, min_ev=0, verbose=False, get_total=True):
    ...
    for train_index, test_index in kf.split(X):
        ...
        # print(f"{count}: {get_ev_from_df(ev_prepped_df, print_stats = False)}")
        count = count + 1
        running_total = running_total + get_ev_for_optimize_mov(
            odds_test, probs, y_test, min_ev=min_ev, get_total=get_total
        )
...
def get_best_features(
    pos_features, m, df, cur_features, labels, odds, scale=False, min_ev=0
):
    best_feature = ""
    ...
    if best_feature != "":
        ...
        return get_best_features(
            pos_features, m, df, cur_features, labels, odds, scale, min_ev=min_ev
        )
    else:
...
def get_best_features_mov(
    pos_features, m, df, cur_features, labels, odds, label_list, scale=False, min_ev=0
):
    best_feature = ""
    ...
    for f in pos_features:
        ...
        if f not in cur_features:
            ...
            new_score = custom_cv_eval_mov(
                df_sel, m, labels_sel, odds_sel, min_ev=min_ev
            )
            # print(f"{len(df_sel)} {len(labels_sel)} {len(odds_sel)}")
            if new_score > best_score:
                print(f"Feature: {f} Score: {new_score}")
...
def tune_LogisticRegression(
    input_model, input_features, input_df, input_labels, odds_input, min_ev=0
):
    ###############################################################################################################
    ...
    best_score = get_ev(
        input_df, input_model, input_features, input_labels, odds_input, min_ev=min_ev
    )
            ...
            for t in tol:
                if (
                    ...
                    test_model = LogisticRegression(
                        solver=s, penalty=p, tol=t, random_state=75, max_iter=50000
                    )
                    score = get_ev(
                        input_df,
                        test_model,
                    ...
                    if score > best_score:
                        ...
                        pass
                        print("solver:", s, "penalty:", p, "tol:", t, "Score:", score)
    return output_model


def tune_DecisionTreeClassifier(
    input_model, input_features, input_df, input_labels, odds_input, min_ev=0
):
    ###############################################################################################################
    # Parameters we are going to fine-tune:
    ...
    min_samples_leaf = [
        input_model.min_samples_leaf,
        input_model.min_samples_leaf * 1.01,
    ...
    if (input_model.max_leaf_nodes == None) or (input_model.max_leaf_nodes == 1):
        ...
        max_leaf_nodes = [
            input_model.max_leaf_nodes,
            input_model.max_leaf_nodes - 1,
            input_model.max_leaf_nodes + 1,
...
def tune_RandomForestClassifier(
    input_model,
    input_features,
    input_df,
    input_labels,
                ...
                for md in max_depth:
                    for mf in max_features:
                        for c in criterion:
                            if (
                                (len(tested_hps) == 6)
                                and (n in tested_hps[0])
                                and (ml in tested_hps[1])
                                and (ms in tested_hps[2])
                                and (md in tested_hps[3])
                                and (mf in tested_hps[4])
                                and (c in tested_hps[5])
                            ):
...
def tune_GradientBoostingClassifier(
    input_model,
    input_features,
    input_df,
    input_labels,
...
def tune_GaussianNB(
    ...
    for v in var_smoothing:
        ...
        if score > best_score:
            ...
            print()
            print("NEW BEST SCORE")
            print("var_smoothing:", v, "Best Score:", best_score)
            print()
            print()
        else:
            pass
            print("var_smoothing:", v, "Score:", score)
    return output_model


def tune_MLPClassifier(
    input_model, input_features, input_df, input_labels, odds_input, min_ev=0
):
    ###############################################################################################################
    # Parameters we are going to fine-tune:
    ...
    hidden_layer_sizes = [
        input_model.hidden_layer_sizes,
        (100,),
        (10, 5),
        (6,),
        (1,),
    ...
    for h in hidden_layer_sizes:
        ...
        if score > best_score:
            ...
            print()
            print("NEW BEST SCORE")
            print("hidden_layer_sizes:", h, "Best Score:", best_score)
            print()
            print()
        else:
            pass
            print("hidden_layer_sizes:", h, "Score:", score)

    return output_model


def tune_RadiusNeighborsClassifier(
    input_model, input_features, input_df, input_labels, odds_input, min_ev=0
):
    ###############################################################################################################
    # Parameters we are going to fine-tune:
    ...
    for w in weights:
        score = -10000
        for p1 in p:
            test_model = RadiusNeighborsClassifier(
                weights=w, p=p1, outlier_label="most_frequent"
            )
            score = get_ev(
                input_df,
                test_model,
            ...
            if score > best_score:
                ...
                print()
                print("NEW BEST SCORE")
                print("weights:", w, "p:", p1, "Best Score:", best_score)
                print()
                print()
            else:
                pass
                print("weights:", w, "p:", p1, "Score:", score)

    return output_model


def tune_KNeighborsClassifier(
    input_model, input_features, input_df, input_labels, odds_input, min_ev=0
):
    ###############################################################################################################
    # Parameters we are going to fine-tune:
    ...
    for w in weights:
        ...
        for n in n_neighbors:
            ...
            if score > best_score:
                ...
                print()
                print("NEW BEST SCORE")
                print("weights:", w, "n_neighbors:", n, "Best Score:", best_score)
                print()
                print()
            else:
                pass
                print("weights:", w, "n_neighbors:", n, "Score:", score)

    return output_model


def tune_SGDClassifier(
    input_model, input_features, input_df, input_labels, odds_input, min_ev=0
):
    ###############################################################################################################
    # Parameters we are going to fine-tune:
    ...
    penalty = ["l1", "l2"]
    alpha = [input_model.alpha, input_model.alpha * 0.9, input_model.alpha * 1.1]
        ...
        for p in penalty:
            ...
            for a in alpha:
                ...
                if score > best_score:
                    ...
                    pass
                    print("loss: ", l, "penalty: ", p, "alpha: ", a, "Score: ", score)

    return output_model


def tune_BaggingClassifier(
    input_model, input_features, input_df, input_labels, odds_input, min_ev=0
):
    ###############################################################################################################
    # Parameters we are going to fine-tune:
    ...
    n_estimators = [
        input_model.n_estimators,
        input_model.n_estimators + 3,
        input_model.n_estimators - 3,
    ]
            ...
            for e in n_estimators:
                if e > 0:
                    test_model = BaggingClassifier(
                        base_estimator=be, bootstrap=bs, n_estimators=e, random_state=75
                    )
                    score = get_ev(
                        input_df,
                        test_model,
...
def tune_LinearDiscriminantAnalysis(
    input_model, input_features, input_df, input_labels, odds_input, min_ev=0
):
    ###############################################################################################################
    # Parameters we are going to fine-tune:
    ...
    for s in solver:
        for t in tol:
            ...
            if score > best_score:
                ...
                print()
                print("NEW BEST SCORE")
                print("solver: ", s, "tol: ", t, "Best Score: ", best_score)
                print()
                print()
            else:
                pass
                print("solver: ", s, "tol: ", t, "Score: ", score)
    ...
    for n in nu:
        for t in tol:
            ...
            if score > best_score:
                ...
                print()
                print("NEW BEST SCORE")
                print("nu: ", n, "tol: ", t, "Best Score: ", best_score)
                print()
                print()
            else:
                pass
                print("nu: ", n, "tol: ", t, "Score: ", score)
    ...
    for n in C:
        for t in tol:
            ...
            if score > best_score:
                ...
                print()
                print("NEW BEST SCORE")
                print("C: ", n, "tol: ", t, "Best Score: ", best_score)
                print()
                print()
            else:
                pass
                print("C: ", n, "tol: ", t, "Score: ", score)
...
def tune_hyperparameters(
    ...
    if isinstance(input_model, SVC):
        while keep_going:
            ...
            if str(pos_model) == str(
                best_model
            ):  # Direct comparisons don't seem to work....
                keep_going = False
                output_model = best_model
            else:
                best_model = pos_model
        ...
        while keep_going:
            pos_model = tune_LinearDiscriminantAnalysis(
                best_model,
                input_features,
                input_df,
                input_labels,
        ...
        while keep_going:
            pos_model = tune_BaggingClassifier(
                best_model,
                input_features,
                input_df,
                input_labels,
        ...
        while keep_going:
            pos_model = tune_LogisticRegression(
                best_model,
                input_features,
                input_df,
                input_labels,
        ...
        while keep_going:
            pos_model = tune_DecisionTreeClassifier(
                best_model,
                input_features,
                input_df,
                input_labels,
        ...
        tested_hps = []
        while keep_going:
            pos_model, tested_hps = tune_RandomForestClassifier(
                best_model,
                input_features,
                input_df,
                input_labels,
        ...
        tested_hps = []
        while keep_going:
            pos_model, tested_hps = tune_ExtraTreeClassifier(
                best_model,
                input_features,
                input_df,
                input_labels,
        ...
        print("HI")
        while keep_going:
            pos_model = tune_GradientBoostingClassifier(
                best_model,
                input_features,
                input_df,
                input_labels,
        ...
        while keep_going:
            pos_model = tune_RadiusNeighborsClassifier(
                best_model,
                input_features,
                input_df,
                input_labels,
        ...
        while keep_going:
            pos_model = tune_KNeighborsClassifier(
                best_model,
                input_features,
                input_df,
                input_labels,
...
# Let's just hold....
def tune_ev_mov(
    input_model, input_features, input_df, input_labels, odds_input, verbose=False
):
    return 0
...
def remove_to_improve_mov(cur_features, m, df, labels, odds, scale=False, min_ev=0):
    ...
    labels_sel = labels[labels.index.isin(df_sel.index)]
    odds_sel = odds[odds.index.isin(df_sel.index)]
    orig_score = custom_cv_eval_mov(
        df_sel, m, labels_sel, odds_sel, get_total=True, min_ev=min_ev
    )
...
def evaluate_model(
    ...
    df_train, df_test = df_train.align(
        df_test, join="left", axis=1
    )  # Ensures both sets are dummified the same
    ...
    prepped_test = list(
        zip(odds_test[:, -2], odds_test[:, -1], probs[:, 0], probs[:, 1], labels_test)
    )
    ev_prepped_df = pd.DataFrame(
        prepped_test, columns=["t1_odds", "t2_odds", "t1_prob", "t2_prob", "winner"]
    )
    ...
    # display(df_test)
    # display(df_test)
    model_score = get_ev_from_df(
        ev_prepped_df, print_stats=True, min_ev=input_ev, get_total=True
    )

    return model_score


def evaluate_model_mov(
    input_model,
    input_features,
    input_ev,
    train_df,
    ...
    probs = input_model.predict_proba(scaled_test)
    model_score = get_ev_from_df_mov(
        odds_test,
        probs,
        labels_test,
        label_list,
        input_model.classes_,
        print_stats=True,
        min_ev=input_ev,
        get_total=True,
    )
